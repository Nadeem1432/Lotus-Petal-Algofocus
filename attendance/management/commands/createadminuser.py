...
class Command(BaseCommand):
    help = 'create users for admin user'
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            group = Group.objects.get_or_create(name='admin')  # group create or get

            ''' NOTE : get all permissions of timetable model '''
            all_permissions = Permission.objects.filter(content_type__app_label='timetable',
                                                        content_type__model='timetable')
            group = Group.objects.get(name='admin')  # group get for add user

            try:
                ''' TODO : add permissions to admin group'''
